...
export default function RoomCard({ room, status }: Props) {
  ...
  // Button content while an action is processing
  const displayButtonContent = actionLoading ? (
    <div className="flex items-center justify-center gap-2">
      <Loader2 className="w-4 h-4 animate-spin" />
      <span>
        {buttonLabel === "Book Now" || buttonLabel === "Check In"
          ? "Loading..."
          : "Processing..."}
      </span>
    </div>
  ) : (
    buttonLabel || room.status
  );

  // Show a full-card overlay during navigation
  const isCardProcessing =
    actionLoading && normalizedStatus !== "checked-in";

  return (
    <div
      className={`relative bg-white rounded-2xl shadow-lg p-5 flex flex-col gap-3 transition-transform ${
        isCardProcessing
