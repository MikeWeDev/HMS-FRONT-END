...
  const Icon = notification.type === 'error' ? AlertCircle : CheckCircle;
    ...
    // FIX 1: Move useEffect BEFORE any conditional return.
...
  }, [notification.message, setNotification]); // Dependency array updated to be more specific (on message change)

  if (!notification.message) return null; // This is now safe, as all hooks have been called.
...
  const availableRooms = useMemo(() => rooms.filter(r => r.status === "Available").length, [rooms]);
    // The bookedRooms variable is now used to fix the unused-vars error.
...
      title: "Occupied/Booked",
        // FIX 2: Use the 'bookedRooms' variable explicitly.
      count: bookedRooms, // was: totalRooms - availableRooms
      icon: CalendarCheck,
