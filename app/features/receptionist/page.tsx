...
import RoomCardRecption from "../../components/RoomCardRecption"; // Corrected import to match the file name
...
        {/* Filter Section */}
        <div className="mb-6">
          {/* Mobile Filter Block: Display as dropdown (Visible below md breakpoint) */}
...
            <select
              value={filterStatus}
              onChange={(e) => setFilterStatus(e.target.value as "All" | Room["status"])}
              className="w-full px-4 py-2 rounded-lg border border-gray-300 bg-white shadow-sm focus:ring-blue-500 focus:border-blue-500"
            >
...
          {/* Desktop Filter Block: Display as buttons (Hidden below md breakpoint) */}
...
            <button
              onClick={() => fetchRooms()}
              className="ml-4 px-3 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition shadow-sm"
            >
              Refresh
            </button>
          </div>
...
              <RoomCardRecption key={room.id} room={room} status={room.status} />
            ))}
          </div>
        )}
      </main>
